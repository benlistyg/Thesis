  ...
  if(n_factors == 2){
    ...
    if(item_type == 'gpcm'){
      ...
      M2_output <- rbind(
        ...
        M2(correctly_fitted, QMC = T, quadpts = NULL, theta_lim = c(-3,3)) %>% mutate(Model = 'Correct'),
        M2(incorrectly_fitted, QMC = T, quadpts = NULL, theta_lim = c(-3,3)) %>% mutate(Model = 'Incorrect')
